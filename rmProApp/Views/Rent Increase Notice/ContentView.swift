    ...
    var body: some View {
        ...
        VStack {
            HStack {
                ...
                Button("Haven Labels") {
                    // Prepare data
                    let filteredUnits: [RMUnit]? = units?
                        .filter { (unit: RMUnit) in unit.propertyID == 3 }
                        .sorted { (lhs: RMUnit, rhs: RMUnit) -> Bool in
                            // Extract the specific user-defined value flag for each unit
                            let lhsFlag = lhs.userDefinedValues?
                                .first(where: { $0.name == "HEI- Fire Protection Approved 2026" })?
                                .value ?? ""
                            let rhsFlag = rhs.userDefinedValues?
                                .first(where: { $0.name == "HEI- Fire Protection Approved 2026" })?
                                .value ?? ""
                            // Put units with a truthy/"approved" value first; customize matching as needed
                            let lhsApproved = lhsFlag.lowercased() == "true" || lhsFlag.lowercased() == "yes" || lhsFlag == "1"
                            let rhsApproved = rhsFlag.lowercased() == "true" || rhsFlag.lowercased() == "yes" || rhsFlag == "1"
                            if lhsApproved != rhsApproved {
                                return lhsApproved && !rhsApproved
                            }
                            // Tie-breaker: sort by unit name if available
...
//                    )
                ...
                Button("Pembroke Labels") {
                    let preFilteredUnits = units?.filter { $0.propertyID == 12 }
                    let filteredUnits = preFilteredUnits?.sorted { unit1, unit2 in
